...
static void write_root_script(void) {
  /* Buffer enlarged 4096 -> 8192: the expanded post-exploit recovery state
   * machine (ksud discovery + late-load + policy fixup + unified enforcing
   * restore + cleanup) no longer fits in the original 4 KiB. */
  char script[8192];
  ...
  int n = snprintf(
      ...
      "\n"
      "# ---- A. ksud discovery: full /data/app scan, then fixed-path fallback ----\n"
      "discover_ksud() {\n"
      "  for CAND in /data/app/*/*/lib/arm64/libksud.so; do\n"
      "    [ -f \"$CAND\" ] || continue\n"
      "    KSUD=\"$CAND\"\n"
      "    echo \"[*] ksud discovered by scan: $KSUD\" >>\"$LOG\"\n"
      "    return 0\n"
      "  done\n"
      "  for P in /data/local/tmp/ksud /data/adb/ksu/bin/ksud; do\n"
      "    if [ -x \"$P\" ]; then\n"
      "      KSUD=\"$P\"\n"
      "      echo \"[*] ksud fallback: $KSUD\" >>\"$LOG\"\n"
      "      return 0\n"
      "    fi\n"
      "  done\n"
      "  return 1\n"
      "}\n"
      "\n"
      "if [ ! -x \"$KSUD\" ]; then discover_ksud; fi\n"
      "echo \"[*] ksud=$KSUD\" >>\"$LOG\"\n"
      ...
      "echo \"[*] KMI=$KMI\" >>\"$LOG\"\n"
      "\n"
      "# ---- D. policy fixup (W1 already made permissive upstream) ----\n"
      "echo \"[*] W1 already made permissive; starting policy fixup\" >>\"$LOG\"\n"
      "FIXUP_RC=1\n"
      ...
      "  if [ \"$FIXUP_RC\" -eq 0 ]; then break; fi\n"
      "  sleep 2\n"
      "done\n"
      ...
      "fi\n"
      "\n"
      "# ---- E. KernelSU state + late-load (with return-code capture) ----\n"
      ...
      "  LATE_RC=$?\n"
      "  echo \"[*] late-load exit=$LATE_RC\" >>\"$LOG\"\n"
      "fi\n"
      "\n"
      "# ---- F. wait for kernelsu.ko (100 x 0.1s = up to 10s, was 5s) ----\n"
      "echo \"[*] watching kernelsu.ko\" >>\"$LOG\"\n"
      "KSU_READY=0\n"
      ...
      "\n"
      "# ---- G. UNIFIED enforcing restore ----\n"
      "# Was: only when KSU_ALREADY=1. Now: whenever the policy fixup succeeded,\n"
      "# regardless of whether KernelSU was pre-loaded or late-loaded by us.\n"
      "# (kernelsu_init also re-enforces; this is the deterministic backstop.)\n"
      "echo \"[*] fixup OK (KSU_ALREADY=$KSU_ALREADY); restoring enforcing\" >>\"$LOG\"\n"
      "sleep 1\n"
      "echo 1 > /sys/fs/selinux/enforce 2>/dev/null\n"
      "RESTORE_RC=$?\n"
      "if [ \"$RESTORE_RC\" -eq 0 ]; then\n"
      "  echo '[+] enforcing restored' >>\"$LOG\"\n"
      "else\n"
      "  echo \"[!] enforcing restore failed rc=$RESTORE_RC\" >>\"$LOG\"\n"
      "fi\n"
      "\n"
      "# ---- H. cleanup ----\n"
      "# Remove only the script itself. .ghostlock_ksu.log is deliberately KEPT:\n"
      "# run_exploit() polls it for the '[+] KernelSU module loaded' marker, so\n"
      "# ---- I. explicit exit status ----\n"
      "if [ \"$RESTORE_RC\" -eq 0 ]; then\n"
      "  exit 0\n"
      "else\n"
      "  exit 2\n"
  ...
  if (n < 0 || n >= (int)sizeof(script)) {
    pr_warning("root script too long (%d bytes, buffer %zu)\n", n, sizeof(script));
    close(sfd);
    return;
  }
